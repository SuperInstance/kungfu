...
download "$LATEST_URL" "$INSTALL_DIR/kungfu"

# Make executable
chmod +x "$INSTALL_DIR/kungfu"
...
echo "Installed to $INSTALL_DIR/kungfu"
...
create_global_symlink() {
    if [ -d "$GLOBAL_BIN" ]; then
        if [ -w "$GLOBAL_BIN" ]; then
            ...
            echo "Created symlink: $GLOBAL_BIN/kungfu"
        elif command -v sudo >/dev/null 2>&1; then
            echo "Creating symlink in $GLOBAL_BIN (requires sudo)..."
            if sudo ln -sf "$INSTALL_DIR/kungfu" "$GLOBAL_BIN/kungfu"; then
                echo "Created symlink: $GLOBAL_BIN/kungfu"
            fi
        fi
    fi
...
install_completions() {
    local shell="$1"
    local rc_file="$2"
    local comp_dir="$HOME/.local/share/kungfu/completions"

    mkdir -p "$comp_dir"

    if download "${COMPLETIONS_URL}/kungfu.${shell}" "$comp_dir/kungfu.${shell}" 2>/dev/null; then
        local source_line="source $comp_dir/kungfu.${shell}"
        if [ -f "$rc_file" ] && ! grep -q "kungfu.${shell}" "$rc_file" 2>/dev/null; then
            echo "" >> "$rc_file"
            echo "# kungfu completions" >> "$rc_file"
...
echo ""
echo "Run 'kungfu --help' to get started"
